...
if (typeof jQuery === "undefined") {
	...
	var js = {
		...
		ie: function() {
			...
			return (!!window.ActiveXObject || "ActiveXObject" in window) ? ((agent.match(/msie\s(\d+)/) || [])[1] || (agent.match(
				/Trident/i) && agent.match(/rv:(\d+)/) || [])[1] || false) : false
		}(),
		...
		showMessage: function(message, title, type, timeout) {
			...
			if (type == "success" || msg.indexOf("成功") >= 0) {
				...
				if (type == "error" || msg.indexOf("错误") >= 0 || msg.indexOf("失败") >= 0) {
					...
					if (type == "warning" || msg.indexOf("不能") >= 0 || msg.indexOf("不允许") >= 0 || msg.indexOf("必须") >= 0 || msg.indexOf(
							"已存在") >= 0 || msg.indexOf("不需要") >= 0 || msg.indexOf("不正确") >= 0) {
						msgType = "warning";
						layerIcon = 5
					} else {
						msgType = "info";
		...
		showErrorMessage: function(responseText) {
			if (responseText && responseText != "") {
				...
				if (responseText.indexOf("<html ") != -1 || responseText.indexOf("<head ") != -1 || responseText.indexOf(
						"<body ") != -1) {
		 ...
		 * 组织机构树结构选择器
		 */
		orgTreeselect : function($this){
			var id = $this.id;
			var $this = $("#"+$this.id);
			var url = $this.data("url")||$this.attr("url");
			var title = $this.data("title")||$this.attr("title");
			var selectRoot = $this.data("selectRoot")||$this.attr("selectRoot");
			...
			var options = {
					...
					success: function(layero, index){
					 ...
					 yes: function(index, layero){	
						...
						win.$('#companyId').val('').change(); 
						win.$('#companyName').val('').change(); 
						
						var tree = win.tree;
						var ids = [], names = [], nodes = [],pIds = [], pNames = [];
						 var checked = "false";
						if ( checked == "true"){
						...
						for(var i=0; i<nodes.length; i++) {
							...
							pNames.push(nodes[i].pName);
							break;
						}
						$("#"+id+"Id").val(ids.join(',')).change();
						$("#"+id+"Name").val(names.join(',')).change();
						$('#companyId').val(pIds.join(',')).change();
						$('#companyName').val(pNames.join(',')).change();
		 ...
		 * 列表选择器
		 */
		listselect:function($this){
			
			var id = $this.id;
			var $this = $("#"+$this.id);
			var url = $this.data("url")||$this.attr("url");
			var title = $this.data("title")||$this.attr("title");
			var boxWidth = $(top.window).width() - 100 < 350 ? 350 : $(top.window).width() - 100;
			var boxHeight = $(top.window).height() - 100 < 250 ? 250 : $(top.window).height() - 100;
			
			//选中的列表数据
			var selectData = {};
			
			if(typeof listselectGetSelectData == 'function'){
				selectData = listselectGetSelectData('roleSelect');
			}else{
				var ids = $('#roleSelectId').val(); 
				var names = $('#roleSelectName').val();
				if(ids != null && ids != "" && names != null && names != ""){
					var idsArr = ids.split(","), namesArr = names.split(",");
					if (idsArr && namesArr && idsArr.length == namesArr.length){
						for(var i=0; i<idsArr.length; i++) {
							selectData[idsArr[i]] = {id: idsArr[i], name: namesArr[i]}
						}
					}
				}
			}
			...
			var options = {
					...
					content: $('#roleSelectDiv').attr('data-url'),
					contentFormData: {
						checkbox: 'true',
					...
					btn1: function(index, layero){
						//var win = js.layer.iframeWindow(index);
						var win = js.layer.iframeWindow(index)[0];
						...
						$.each(selectData, function(key, value){
				   		...
				   		$('#roleSelectId').val(codes.join(',')).change();
						if(typeof listselectCallback == 'function'){
							listselectCallback('roleSelect', 'ok', index, layero, selectData);
						}
					}
				};
				...
				options['btn'+options.btn.length] = function(index, layero){
					if(typeof listselectCallback == 'function'){
						listselectCallback('roleSelect', 'cancel', index, layero);
					}
				};
				js.layer.open(options);
			},
	...
	window.text = js.text
})(window.jQuery, window); + function(d) {
	var b = "lte.boxwidget";
	var g = {
		animationSpeed: 500,
				...
				if (!c) {
					...
					if (b.isUseSpan == false) {
						...
						var m = a(
							...
							b.placeholderColor + "; width:" + k + "px; height:" + l + "px;" + f + '">' + i + "</span>");
						m.css({
...
(function(X) {
	...
	X.fn[R] = function(D, v) {
		...
		if (/^(check|uncheck|toggle|indeterminate|determinate|disable|enable|update|destroy)$/i.test(D)) {
			return D = D.toLowerCase(), q(this), t.each(function() {
				var k = X(this);
		...
		return t.each(function() {
			...
			L.on("click.i focus.i blur.i keyup.i keydown.i keypress.i", function(k) {
				...
				if ("keydown" == Y && 32 == k) {
					return I[T] == K && I[V] || (I[V] ? O(L, V) : Q(L, V)), !1
				}
				if ("keyup" == Y && I[T] == K) {
					!I[V] && Q(L, V)
				} else {
...
(function(f, d, a) {
	...
	b.prototype = {
		...
		getUTCDate: function(i) {
			var h;
			i = i || new Date();
			h = Date.UTC(i.getUTCFullYear(), i.getUTCMonth(), i.getUTCDate(), i.getUTCHours(), i.getUTCMinutes(), i.getUTCSeconds(),
				...
				try {
					if (m.extraData) {
						for (var d in m.extraData) {
							m.extraData.hasOwnProperty(d) && l.push(e.isPlainObject(m.extraData[d]) && m.extraData[d].hasOwnProperty(
								"name") && m.extraData[d].hasOwnProperty("value") ? e('<input type="hidden" name="' + m.extraData[d].name +
								'">').val(m.extraData[d].value).appendTo(w)[0] : e('<input type="hidden" name="' + d + '">').val(m.extraData[
								d]).appendTo(w)[0])
						}
					}
					...
					if (M && M.location.href != m.iframeSrc || T) {
						...
						try {
							...
							if (l || m.textarea) {
								...
								if (f) {
									x.responseText = f.value, x.status = Number(f.getAttribute("status")) || x.status, x.statusText = f.getAttribute(
										"statusText") || x.statusText
								} else {
									if (l) {
										var p = M.getElementsByTagName("pre")[0],
											h = M.getElementsByTagName("body")[0];
										p ? x.responseText = p.textContent ? p.textContent : p.innerText : h && (x.responseText = h.textContent ? h
...
if (!js.ie) {
		...
		1: [function(b, a) {
			! function() {
				function h(u, y) {
					...
					var w;
					if (y = y || {}, this.trackingClick = !1, this.trackingClickStart = 0, this.targetElement = null, this.touchStartX =
						0, this.touchStartY = 0, this.lastTouchIdentifier = 0, this.touchBoundary = y.touchBoundary || 10, this.layer =
						u, this.tapDelay = y.tapDelay || 200, this.tapTimeout = y.tapTimeout || 700, !h.notNeeded(u)) {
						for (var v = ["onMouse", "onClick", "onTouchStart", "onTouchMove", "onTouchEnd", "onTouchCancel"], p = this, x =
								0, m = v.length; m > x; x++) {
							p[v[x]] = q(p[v[x]], p)
						}
						g && (u.addEventListener("mouseover", this.onMouse, !0), u.addEventListener("mousedown", this.onMouse, !0), u.addEventListener(
								"mouseup", this.onMouse, !0)), u.addEventListener("click", this.onClick, !0), u.addEventListener(
								"touchstart", this.onTouchStart, !1), u.addEventListener("touchmove", this.onTouchMove, !1), u.addEventListener(
								"touchend", this.onTouchEnd, !1), u.addEventListener("touchcancel", this.onTouchCancel, !1), Event.prototype
							.stopImmediatePropagation || (u.removeEventListener = function(s, t, o) {
							...
							}), "function" == typeof u.onclick && (w = u.onclick, u.addEventListener("click", function(i) {
								w(i)
							}, !1), u.onclick = null)
					}
				}
				...
				h.prototype.needsClick = function(i) {
						...
						var o, l;
						document.activeElement && document.activeElement !== m && document.activeElement.blur(), l = i.changedTouches[
							0], o = document.createEvent("MouseEvents"), o.initMouseEvent(this.determineEventType(m), !0, !0, window, 1,
							l.screenX, l.screenY, l.clientX, l.clientY, !1, !1, !1, !1, 0, null), o.forwardedTouchEvent = !0, m.dispatchEvent(
							o)
						...
						var i;
						d && l.setSelectionRange && 0 !== l.type.indexOf("date") && "time" !== l.type && "month" !== l.type ? (i = l.value
							.length, l.setSelectionRange(i, i)) : l.focus()
						...
						if (i = this.getTargetElementFromEventTarget(m.target), o = m.targetTouches[0], d) {
							...
							if (!j) {
								...
								this.lastTouchIdentifier = o.identifier, this.updateScrollParent(i)
							}
						}
						return this.trackingClick = !0, this.trackingClickStart = m.timeStamp, this.targetElement = i, this.touchStartX =
							o.pageX, this.touchStartY = o.pageY, m.timeStamp - this.lastClickTime < this.tapDelay && m.preventDefault(),
							!0
						...
						return this.trackingClick ? ((this.targetElement !== this.getTargetElementFromEventTarget(i.target) || this.touchHasMoved(
							i)) && (this.trackingClick = !1, this.targetElement = null), !0) : !0
					}, h.prototype.findControl = function(i) {
						return void 0 !== i.control ? i.control : i.htmlFor ? document.getElementById(i.htmlFor) : i.querySelector(
						...
						if (this.cancelNextClick = !1, this.lastClickTime = q.timeStamp, v = this.trackingClickStart, this.trackingClick = !
							1, this.trackingClickStart = 0, f && (i = q.changedTouches[0], o = document.elementFromPoint(i.pageX - window
								.pageXOffset, i.pageY - window.pageYOffset) || o, o.fastClickScrollParent = this.targetElement.fastClickScrollParent),
							m = o.tagName.toLowerCase(), "label" === m) {
							...
							if (this.needsFocus(o)) {
								return q.timeStamp - v > 100 || d && window.top !== window && "input" === m ? (this.targetElement = null, !1) :
									(this.focus(o), this.sendClick(o, q), d && "select" === m || (this.targetElement = null, q.preventDefault()),
										!1)
							}
						}
						return d && !j && (s = o.fastClickScrollParent, s && s.fastClickLastScrollTop !== s.scrollTop) ? !0 : (this.needsClick(
							o) || (q.preventDefault(), this.sendClick(o, q)), !1)
					}, h.prototype.onTouchCancel = function() {
						this.trackingClick = !1, this.targetElement = null
					}, h.prototype.onMouse = function(i) {
						return this.targetElement ? i.forwardedTouchEvent ? !0 : i.cancelable && (!this.needsClick(this.targetElement) ||
							this.cancelNextClick) ? (i.stopImmediatePropagation ? i.stopImmediatePropagation() : i.propagationStopped = !
							0, i.stopPropagation(), i.preventDefault(), !1) : !0 : !0
					}, h.prototype.onClick = function(l) {
						var i;
						return this.trackingClick ? (this.targetElement = null, this.trackingClick = !1, !0) : "submit" === l.target.type &&
							0 === l.detail ? !0 : (i = this.onMouse(l), i || (this.targetElement = null), i)
					}, h.prototype.destroy = function() {
						var i = this.layer;
						g && (i.removeEventListener("mouseover", this.onMouse, !0), i.removeEventListener("mousedown", this.onMouse, !
								0), i.removeEventListener("mouseup", this.onMouse, !0)), i.removeEventListener("click", this.onClick, !0), i
							.removeEventListener("touchstart", this.onTouchStart, !1), i.removeEventListener("touchmove", this.onTouchMove,
								!1), i.removeEventListener("touchend", this.onTouchEnd, !1), i.removeEventListener("touchcancel", this.onTouchCancel,
								!1)
						...
						if (c && (l = navigator.userAgent.match(/Version\/([0-9]*)\.([0-9]*)/), l[1] >= 10 && l[2] >= 3 && (m =
						...
						return "none" === p.style.msTouchAction || "manipulation" === p.style.touchAction ? !0 : (q = +(
								/Firefox\/([0-9]+)/.exec(navigator.userAgent) || [, 0])[1], q >= 27 && (m = document.querySelector(
								"meta[name=viewport]"), m && (-1 !== m.content.indexOf("user-scalable=no") || document.documentElement.scrollWidth <=
								window.outerWidth)) ? !0 : "none" === p.style.touchAction || "manipulation" === p.style.touchAction ? !0 :
							!1)
					}, h.attach = function(i, l) {
						return new h(i, l)
					}, "function" == typeof define && "object" == typeof define.amd && define.amd ? define(function() {
						return h
					}) : "undefined" != typeof a && a.exports ? (a.exports = h.attach, a.exports.FastClick = h) : window.FastClick =
					h
			}()
		}, {}],
		2: [function(a) {
	...
	a.fn.dropdownHover = function(d) {
		return "ontouchstart" in document ? this : (b = b.add(this.parent()), this.each(function() {
			function f() {
				m.parents(".navbar").find(".navbar-toggle").is(":visible") || (c.clearTimeout(o), c.clearTimeout(j), j = c.setTimeout(
					function() {
						b.find(":focus").blur(), p.instantlyCloseOthers === !0 && b.removeClass("open"), c.clearTimeout(j), m.attr(
							"aria-expanded", "true"), t.addClass("open"), m.trigger(k)
					}, p.hoverDelay))
			}
			var o, j, m = a(this),
				t = m.parent(),
			...
			t.hover(function(h) {
				...
				c.clearTimeout(j), o = c.setTimeout(function() {
					m.attr("aria-expanded", "false"), t.removeClass("open"), m.trigger(n)
				}, p.delay)
			}), m.hover(function(h) {
				...
				var i, h = a(this);
				h.hover(function() {
					c.clearTimeout(i), h.children(".dropdown-menu").show(), h.siblings().children(".dropdown-menu").hide()
				}, function() {
...
! function() {
	var d = {
		...
		b = {
			...
			escape: function(c) {
				return String(c || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(
					/'/g, "&#39;").replace(/"/g, "&quot;")
			},
			error: function(h, c) {
				var i = "Laytpl Error：";
	...
	a.pt = a.prototype, window.errors = 0, a.pt.parse = function(k, q) {
		var n = this,
			m = k,
			i = g("^" + d.open + "#", ""),
			h = g(d.close + "$", "");
		k = k.replace(/\s+|\r|\t|\n/g, " ").replace(g(d.open + "#"), d.open + "# ").replace(g(d.close + "}"), "} " + d.close)
			.replace(/\\/g, "\\\\").replace(/(?="|')/g, "\\").replace(b.query(), function(c) {
				return c = c.replace(i, "").replace(h, ""), '";' + c.replace(/\\/g, "") + ';view+="'
			}).replace(b.query(1), function(l) {
				var c = '"+(';
				return l.replace(/\s/g, "") === d.open + d.close ? "" : (l = l.replace(g(d.open + "|" + d.close), ""), /^=/.test(
					l) && (l = l.replace(/^=/, ""), c = '"+_escape_('), c + l.replace(/\\/g, "") + ')+"')
			}), k = '"use strict";var view = "' + k + '";return view;';
		try {
			return n.cache = k = new Function("d, _escape_", k), k(q, b.escape)
		} catch (j) {
			return delete n.cache, b.error(j, m)
		}
	}, a.pt.render = function(i, k) {
		var h, j = this;
		return i ? (h = j.cache ? j.cache(i, b.escape) : j.parse(j.tpl, i), console.log(), k ? void k(h) : h) : b.error(
			"no data")
	};
	var f = function(c) {
	...
	}, f.v = "1.2", "function" == typeof define ? define(function() {
		return f
