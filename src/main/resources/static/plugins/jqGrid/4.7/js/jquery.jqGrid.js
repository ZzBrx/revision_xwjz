    ...
    a.jgrid.extend({
        setTreeNode: function(c, b) {
            return this.each(function() {
                ...
                var j = p.p.expColInd,
                l = p.p.treeReader.expanded_field,
                s = p.p.treeReader.leaf_field,
                d = p.p.treeReader.level_field,
                r = p.p.treeReader.icon_field,
                n = p.p.treeReader.loaded,
                u, k, e, h, m, v, t, i;
                while (c < b) {
                    var g = a.jgrid.stripPref(p.p.idPrefix, p.rows[c].id),
                    f = p.p._index[g],
                    q;
                    t = p.p.data[f];
                    ...
                    a(p.rows[c].cells[j]).find("div.treeclick").bind("click",
                    function(y) {
                        var x = y.target || y.srcElement,
                        A = a.jgrid.stripPref(p.p.idPrefix, a(x, p.rows).closest("tr.jqgrow")[0].id),
                        z = p.p._index[A];
                        if (a(this).hasClass("tree-parent")) {
                            a(this).removeClass("tree-parent");
                            if (a(this).hasClass("tree-leaf")) {
                                a(this).removeClass(p.p.treeIcons.leaf + " tree-leaf").addClass(p.p.treeIcons.minus + " tree-minus");
                                p.p.data[z][s] = false
                            }
                        }
                        if (!p.p.data[z][s]) {
                            if (p.p.data[z][l]) {
                                a(p).jqGrid("collapseRow", p.p.data[z]);
                                a(p).jqGrid("collapseNode", p.p.data[z])
                            } else {
                                if (a(this).hasClass("tree-reload")) {
                                    a(this).removeClass("tree-reload");
                                    var w = p.p.data[z];
                                    w[p.p.treeReader.loaded] = false;
                                    a.each(a(p).jqGrid("getNodeChildren", w),
                                    function(B, C) {
                                        C[p.p.treeReader.loaded] = false;
                                        a(p).jqGrid("delTreeNode", C[p.p.localReader.id])
                                    })
                                }
                                a(p).jqGrid("expandRow", p.p.data[z]);
                                a(p).jqGrid("expandNode", p.p.data[z])
                            }
                        }
                        return false
                    });
                    if (p.p.ExpandColClick === true) {
                        a(p.rows[c].cells[j]).find("span.cell-wrapper").css("cursor", "pointer").bind("click",
                        function(x) {
                            var w = x.target || x.srcElement,
                            z = a.jgrid.stripPref(p.p.idPrefix, a(w, p.rows).closest("tr.jqgrow")[0].id),
                            y = p.p._index[z];
                            if (!p.p.data[y][s]) {
                                if (p.p.data[y][l]) {
