...
+function(d) {
	...
	g.prototype.init = function() {
		if (this.options.expandOnHover
				|| (d("body").is(a.mini + a.layoutFixed))) {
			this.expandOnHover();
			d("body").addClass(h.expandFeature)
		}
		d(a.contentWrapper).click(
				function() {
					if (d(window).width() <= this.options.collapseScreenSize
							&& d("body").hasClass(h.open)) {
						this.close()
					}
				}.bind(this));
	...
	g.prototype.close = function() {
		...
		if (j > this.options.collapseScreenSize) {
			...
			d("body").removeClass(h.open + " " + h.collapsed).trigger(
					d.Event(i.collapsed))
		}
	};
	g.prototype.expandOnHover = function() {
		d(a.mainSidebar)
				.hover(
						function() {
							if (d("body").is(a.mini + a.collapsed)
									&& d(window).width() > this.options.collapseScreenSize) {
								this.expand()
							}
						}.bind(this), function() {
		...
		+ function(e) {
			...
			d.prototype.toggle = function(n, m) {
				...
				if (l.children().length === 0) {
					e(".active", this.element).removeClass(
							"treeview-item active");
					j.parents(".treeview:not(.active)").addClass(
							"menu-open active");
			...
			function g(j) {
				return this.each(function() {
					...
					var l = m.data(b);
					if (!l) {
						var k = e.extend({}, f, m.data(), typeof j == "object"
								&& j);
						m.data(b, new d(m, k))
					}
				})
	...
	js.initTabPage("tabpanel",
			{
				height : function() {
					var d = $(window).height(), 
					    b = $(".main-header:visible").outerHeight(), 
					    c = $(".main-footer:visible").outerHeight(), 
					    a = d - b - c;
					return a < 300 ? 300 : a
				}
			});
	$(window).resize();
	js.addTabPage(null, '<i class="fa fa-home"></i> 仪表盘', "/menuList", false, false);
	
	window.isMenuClickFlag = false;
	$(document).on("click", "a.addTabPage", function(a) {
		window.isMenuClickFlag = true
	});
	$(window).bind("hashchange", function(c) {
		if (!window.isMenuClickFlag) {
			var b = window.location.hash.replace("#", "");
			if (b && b != "" && b != window.location.pathname) {
				var a = $('a.addTabPage[href="' + b + '"]:eq(0)');
				if (a && a.length > 0) {
					a.click()
				} else {
					js.addTabPage(null, "新建页签", b)
				}
			} else {
